...
//Test Case #1, testing sf_fragmentation function
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) 
{
    sf_malloc(sizeof(double) * 2);
	sf_malloc(sizeof(int) * 4);
	double fragmentation = sf_fragmentation();
	cr_assert(fragmentation == 0.5, "sf_fragmentation return was not what was expected (found=%lf, exp=%lf)!", fragmentation, (double)0.5);
}

//Test Case #2, testing sf_utilization function
Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT) 
{
	sf_malloc(sizeof(sf_block) * 16);
	double peak_utilization = sf_utilization();
	cr_assert(peak_utilization == 0.125, "sf_utilization return was not what was expected (found=%lf, exp=%lf)!", peak_utilization, (double)0.125);
}
//Test Case #3, testing multiple allocation of ints sizes
Test(sfmm_basecode_suite, student_test_3, .timeout = TEST_TIMEOUT) {
    ...
    cr_assert_not_null(arr, "arr is NULL!");

    for (int i = 0; i < 10; i++) {
        arr[i] = i;
        cr_assert(arr[i] == i, "sf_malloc failed to give proper space for multiple ints!");
    }
...
//Test Case #4, testing double coalescing in free
Test(sfmm_basecode_suite, student_test_4, .timeout = TEST_TIMEOUT) {
    ...
    // Expected coalescing: blocks b, c, and d should merge into one large free block.
...
//Test Case #5, tests if realloc call on a block of the same size returns the same block
Test(sfmm_basecode_suite, student_test_5, .timeout = TEST_TIMEOUT) {
	void *x = sf_malloc(sizeof(double) * 2);
	void *y = sf_realloc(x, sizeof(double) * 2);
